...
store_dat <- mutate(data_00,`Postal_Code` = normalize_zip(`Postal_Code`), 
                    ...
                    `Fulfillment` =  as.numeric (difftime(`Ship_Date`, `Order_Date`, units = "days")), 
...
cat("1-way Contingency Table for Segment", "\n")
table(store_dat$Segment)
cat("\n")
...
cat("1-way Contingency for Table", "\n")
...
cat("2-way Contingency Table for Category and Segment ", "\n")
table(store_dat$Category, store_dat$Segment)
cat("\n")
...
cat("Fulfillment Time by State ", "\n")
store_dat |> 
  select(Fulfillment, State) |> 
  group_by(State) |>
  summarize(min = min(Fulfillment),
            q1 = quantile(Fulfillment, 0.25),
            median = median(Fulfillment),
            mean = mean(Fulfillment),
            q3 = quantile(Fulfillment, 0.75),
            max = max(Fulfillment))
...
states <- states(cb = TRUE, class = "sf") |>
  ...
  mutate(var = aggregate(store_dat$Sales, list(store_dat$State), FUN = sum))
